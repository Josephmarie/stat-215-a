...
def collate(batch):
    """
    Collate function to be used with the DataLoader. It pads the images in the batch to have the same size.
    ...
    # Pad images to have the same height and width
    padded_features = []
    padded_labels = []
    for features, labels in batch:
        pad_height = max_height - features.shape[1]
        pad_width = max_width - features.shape[2]
        padded_features.append(F.pad(features, (0, pad_width, 0, pad_height)))
        padded_labels.append(F.pad(labels, (0, pad_width, 0, pad_height)))
